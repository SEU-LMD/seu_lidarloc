...
static double PI1 = 3.14159265358979323846;
...
#define MIN_LAT ((-80.5 * PI1) / 180.0) /* -80.5 degrees in radians    */
#define MAX_LAT ((84.5 * PI1) / 180.0)  /* 84.5 degrees in radians     */
...
long Convert_Geodetic_To_UTM(double Latitude, double Longitude, long *Zone, char *Hemisphere, double *Easting,
    ...
    if ((Longitude < -PI1) || (Longitude > (2 * PI1))) { /* Longitude out of range */
        Error_Code |= UTM_LON_ERROR;
    }
    if (!Error_Code) { /* no errors */
        if ((Latitude > -1.0e-9) && (Latitude < 0)) Latitude = 0.0;
        if (Longitude < 0) Longitude += (2 * PI1) + 1.0e-10;

        Lat_Degrees = (long)(Latitude * 180.0 / PI1);
        Long_Degrees = (long)(Longitude * 180.0 / PI1);

        if (Longitude < PI1) temp_zone = (long)(31 + ((Longitude * 180.0 / PI1) / 6.0));
        else
            temp_zone = (long)(((Longitude * 180.0 / PI1) / 6.0) - 29);
        ...
        if (!Error_Code) {
            if (temp_zone >= 31) Central_Meridian = (6 * temp_zone - 183) * PI1 / 180.0;
            else
                Central_Meridian = (6 * temp_zone + 177) * PI1 / 180.0;
            *Zone = temp_zone;
            if (Latitude < 0) {
...
long Convert_UTM_To_Geodetic(long Zone, char Hemisphere, double Easting, double Northing, double *Latitude,
    ...
    if ((Northing < MIN_NORTHING) || (Northing > MAX_NORTHING)) Error_Code |= UTM_NORTHING_ERROR;
    if (!Error_Code) { /* no errors */
        if (Zone >= 31) Central_Meridian = ((6 * Zone - 183) * PI1 / 180.0 /*+ 0.00000005*/);
        else
            Central_Meridian = ((6 * Zone + 177) * PI1 / 180.0 /*+ 0.00000005*/);
