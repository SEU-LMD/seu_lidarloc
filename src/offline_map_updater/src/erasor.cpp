...
void ERASOR::voi2r_pod(
    ...
    //r_pod中有占用率的bin都放进curr_init，并发布这个话题
...
//将规定范围内的z与半径r的点存入r_pod，范围之外的存入complement.points
void ERASOR::voi2r_pod(
        const pcl::PointCloud<pcl::PointXYZI> &src,
...
/***
 * 即计算最低的10个点的平均值，将低于平均值+阈值的点作为种子点返回
 * @param p_sorted
 * @param init_seeds
 */
 ...
 *   2.1.1 map_h大于curr_h的时候，将当前帧直接作为selected;提取地图中的地面点push到selected;非地面点作为rejected;
 *   2.1.2 curr_h大于map_h的时候，地图直接作为selected;地图点作为selected,当前帧的点放到rejected;
 *   2.2 当大于0.22.时，即作为selected;
 * 3.当只有当前帧是occupied时，当前帧就加进occupied;
 * 4.当只有地图是occupied时，当前帧就加进occupied;
 *
 * @param frame
 */
        ...
        for (int r = 0; r < num_rings; r++) {
            ...
            if (bin_curr.is_occupied && bin_map.is_occupied) {
                ...
                if (scan_ratio < scan_ratio_threshold) { // find dynamic!
                    ...
                    Bin bin_merged;
                    //把当前帧的bin 和map的bin进行merge,却两者中更大的max_h和两者中更小的min_h;
...
void ERASOR::compare_vois_and_revert_ground_w_block(int frame) {
    ...
    // 1. Update status!!各个bin占用率情况，找到动态点
    for (int theta = 0; theta < num_sectors; theta++) {
        for (int r = 0; r < num_rings; r++) {
            // Min. num of pts criteria.
            Bin &bin_curr = r_pod_curr[r][theta];
            ...
            //静态，直接存入r_pod_selected
             {
                ...
                if ((bin_map.max_h - bin_map.min_h) > 0.5) {
                    ...
                    if (!piecewise_ground_.empty()) piecewise_ground_.clear();
                    if (!non_ground_.empty()) non_ground_.clear();
                    //map拟合地面存入piecewise_ground_，非地面点non_ground_
                    extract_ground(bin_map.points, piecewise_ground_, non_ground_);
                    /*** It potentially requires lots of memories... */
                    //拟合地面存入r_pod_selected
                    r_pod_selected[r][theta].points += piecewise_ground_;
...
void ERASOR::get_static_estimate(
    ...
    // r_pod_selecte被遍历，找到有占用率的点，存到arranged
