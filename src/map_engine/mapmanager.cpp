...
//
#ifndef SERIALIZE_MAP_MANAGER_H
#define SERIALIZE_MAP_MANAGER_H
...
//获得初始变量，对标志位赋值
    void
    MapmanagerInitialized(const std::string &map_read_path) {
...
//小grid索引映射
    int
    down_grid_index2int(const std::string &down_grid_index) {
...
//输出需要加载的大grid编号  小格子到大格子的映射
    ...
    MallocMeomery(const std::string &up_grid_string, const std::string &feature_string) {  //有待改进成完全由超参数定义数据大小
        ...
        lasercloud_index[0] = (i * SerializeConfig::up2down_num) +
                              (j * SerializeConfig::up2down_num * SerializeConfig::lasercloud_width);
...
//输入为大grid
    void
    UpdateTopGrid(const std::string &map_read_path, const std::string &index, const std::string &feature) {
...
//        kdtree_surf_from_map->setInputCloud(laser_cloud_surf_from_map);
    ...
    void process(const Gnsspostion &gnsspostion) {
        ...
        int center_cubeI = static_cast<int>(std::floor(gnsspostion.x - x_min_t) /
                                            (SerializeConfig::up_grid_size / SerializeConfig::up2down_num));
        int center_cubeJ = static_cast<int>(std::floor(gnsspostion.y - y_min_t) /
                                            (SerializeConfig::up_grid_size / SerializeConfig::up2down_num));
