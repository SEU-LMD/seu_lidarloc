...
public:
    ...
    LocmapOptimization() {
        std::cout<<"init Loc optimization function"<<std::endl;
...
//                "lio_sam_6axis/mapping/map_global", 1);
        ...
        subCloud = nh.subscribe<lio_sam_6axis::cloud_info>(
                ...
                &LocmapOptimization::laserCloudInfoHandler, this,
                ros::TransportHints().tcpNoDelay());

        subGPS = nh.subscribe<nav_msgs::Odometry>(
                "gps_odom", 200, &LocmapOptimization::gpsHandler, this,
                ros::TransportHints().tcpNoDelay());

        subLoop = nh.subscribe<std_msgs::Float64MultiArray>(
                "lio_loop/loop_closure_detection", 1, &LocmapOptimization::loopInfoHandler,
                this, ros::TransportHints().tcpNoDelay());
        srvSaveMap = nh.advertiseService("lio_sam_6axis/save_map",
                                         &LocmapOptimization::saveMapService, this);
        ...
        downSizeFilterCorner.setLeafSize(
                Config::mappingCornerLeafSize, Config::mappingCornerLeafSize, Config::mappingCornerLeafSize);
        downSizeFilterSurf.setLeafSize(Config::mappingSurfLeafSize, Config::mappingSurfLeafSize,
                                       Config::mappingSurfLeafSize);
        downSizeFilterICP.setLeafSize(Config::mappingSurfLeafSize, Config::mappingSurfLeafSize,
                                      Config::mappingSurfLeafSize);
        downSizeFilterSurroundingKeyPoses.setLeafSize(
                Config::surroundingKeyframeDensity, Config::surroundingKeyframeDensity,
                Config::surroundingKeyframeDensity);  // for surrounding key poses of
        // scan-to-map optimization
...
//         laserCloudRawKeyFrames);
    ...
    void publishGlobalMap() {
        ...
        kdtreeGlobalMap->radiusSearch(
                cloudKeyPoses3D->back(), Config::globalMapVisualizationSearchRadius,
        ...
        downSizeFilterGlobalMapKeyPoses.setLeafSize(
                Config::globalMapVisualizationPoseDensity, Config::globalMapVisualizationPoseDensity,
                Config::globalMapVisualizationPoseDensity);  // for global map visualization
        ...
        downSizeFilterGlobalMapKeyFrames.setLeafSize(
                Config::globalMapVisualizationLeafSize, Config::globalMapVisualizationLeafSize,
                Config::globalMapVisualizationLeafSize);  // for global map visualization
        downSizeFilterGlobalMapKeyFrames.setInputCloud(globalMapKeyFrames);
        downSizeFilterGlobalMapKeyFrames.filter(*globalMapKeyFramesDS);
        publishCloud(pubLaserCloudSurround, globalMapKeyFramesDS,
                     timeLaserInfoStamp, Config::odometryFrame);
    }

    void loopClosureThread() {
        if (Config::loopClosureEnableFlag == false) return;
        ...
        while (ros::ok()) {
            ...
            if (Config::useGPS) visualGPSConstraint();
    ...
    void performLoopClosure() {
        ...
        *copy_cloudKeyPoses6D = *cloudKeyPoses6D;
        ...
        if (icp.hasConverged() == false ||
            icp.getFitnessScore() > Config::historyKeyframeFitnessScore)
            return;
    ...
    bool detectLoopClosureDistance(int *latestID, int *closestID) {
        ...
        kdtreeHistoryKeyPoses->radiusSearch(
                copy_cloudKeyPoses2D->back(), Config::historyKeyframeSearchRadius,
    ...
    void updateInitialGuess() {
        ...
        /**前一帧的初始化姿态角（来自原始imu数据），用于估计第一帧的位姿（旋转部分）comment by wxy*/
...
/**当前帧位姿的旋转部分，用激光帧信息中的RPY（来自imu原始数据）初始化  comment by wxy*/
...
//---------------------------use gnss for pose guess
        /***
           * 1.cloudInfo来自featureExtraction.cpp发布的lio_sam/feature/cloud_info,
           * 2.而其中的initialGuessX等信息本质上来源于ImageProjection.cpp发布的deskew/cloud_info信息，
           * 3.而deskew/cloud_info中的initialGuessX则来源于ImageProjection.cpp中的回调函数odometryHandler
           * 4.odometryHandler订阅的是imuPreintegration.cpp发布的odometry/imu_incremental话题
           * 5.该话题发布的xyz是imu在前一帧雷达基础上的增量位姿
           * notes:增量位姿，指的绝不是预积分位姿！！是在前一帧雷达的基础上(包括该基础!!)的（基础不是0）的位姿
           * 当前帧的初始估计位姿（来自imu里程计），后面用来计算增量位姿变换
           * comment by wxy
           * 故该部分需要通过激光帧前一帧与当前帧所对应的GNSS里程计的数据来计算一个增量位姿（即位姿变换），然后通过增量位姿来计算当前激光帧的位姿，并进行更新
           */
...
//        }
  /***
        * 1.当cloudInfo.odomAvailable不可获得的时候，才会被调用，这里直接拿imu原始数据的roll、pitch、yaw
        * 2.imuPitchInit是瞬时原始数据，而上述的cloudInfo.initialGuessX是imu里程计发布的姿态角；
        * comment by wxy
        */
...
//            if (cloudToExtract->size() <= Config::surroundingKeyframeSize)
...
//    }
    ...
    /**
     * 1.对最近的一帧关键帧搜索时空纬度上相邻的关键帧集合，降采样一下
     * 2.对关键帧集合中的每一帧提取对应的角点、平面点，加入局部map中
     * note:本函数通过建立KD-TREE来搜索最近邻的关键帧并提取角点和面点，来构建局部地图，此部分可以由离线地图提供，因此可以注释
     *comment by wxy
     * 此部分需提供离线地图
     */
  //  void extractNearby() {
...
//--------- change lidar points to GNSS axis with imu prior->transformTobeMapped
            pointOri = laserCloudCornerLastDS->points[i];
            ////将点转换到gnss map系下
            //把pointOri的点转换到pointSel下，pointSel作为输出
            pointAssociateToMap(&pointOri, &pointSel);
            //kd树最近邻搜索，找到世界坐标系下最近的5个点，要求小于1m
            kdtreeCornerFromMap->nearestKSearch(pointSel, 5, pointSearchInd,
                                                pointSearchSqDis);
            /***
             * 求5个样本的均值，协方差矩阵，对协方差矩阵进行特征分解，如果特征值大于次特征值的3倍，那么就认为构成线
             */

//            cal A1 corvariance to find line
...
//                其中一个特征值比剩下两个都要大，那么这个大的特征值所对应的特征向量代表直线的方向
                //与较大特征值相对应的特征向量代表边缘线的方向（一大两小，大方向），
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1)) {
                  ...
                  //通过coeff来保存“鲁棒后”的距离，和鲁棒后的点到线的方向向量
                    coeff.x = s * la;
                    coeff.y = s * lb;
                    coeff.z = s * lc;
                    coeff.intensity = s * ld2;// intensity本质上构成了一个核函数，ld2越接近于1，增长越慢
                   //如果点到直线的距离小于1m
                    if (s > 0.1) {
                        laserCloudOriCornerVec[i] = pointOri;//存入原始点云
                        coeffSelCornerVec[i] = coeff;//存入鲁棒距离和鲁棒向量
...
//            change points to gnss map
           //将点转换到gnss map系下
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                //假设平面方程为ax+by+cz+1=0，这里就是求方程的系数abc，d=1 （QR分解）
                float pa = matX0(0, 0);
                float pb = matX0(1, 0);
                float pc = matX0(2, 0);
                float pd = 1;
...
//                  laserCloudOriSurfVec, points in lidar frame
//                  coeffSelSurfVec
                    if (s > 0.1) {
                        laserCloudOriSurfVec[i] = pointOri;// 当前激光帧平面点，加入匹配集合中
                        coeffSelSurfVec[i] = coeff;//鲁棒距离和鲁棒向量
                        laserCloudOriSurfFlag[i] = true;//当前点已经找到对应的面了
                    }
                }
            }
        }
...
//        为何需要转换？
        ...
        if (laserCloudSelNum < 50) {
            EZLOG(INFO) << "laserCloudSelNum is too small, num: " << laserCloudSelNum << endl;
            return false;
        }
    ...
    void scan2MapOptimization() {
       ...
       //只有在数目足够多的时候才进行处理，默认最少要10个角点，100个平面点。
        if (laserCloudCornerLastDSNum > Config::edgeFeatureMinValidNum &&
            laserCloudSurfLastDSNum > Config::surfFeatureMinValidNum) {
            /***
             * 构建kd-tree，
             */
...
//    IMU roll pitch yaw will constraint the result comment by ssssyyyy
/***
 *
...
//       更新当前帧的 roll pitch  z
        transformTobeMapped[0] =
                constraintTransformation(transformTobeMapped[0], Config::rotation_tollerance);
        transformTobeMapped[1] =
                constraintTransformation(transformTobeMapped[1], Config::rotation_tollerance);
        transformTobeMapped[5] =
                constraintTransformation(transformTobeMapped[5], Config::z_tollerance);
//      incrementalOdometryAffineBack 当前帧位姿
    ...
    bool saveFrame() {
        if (cloudKeyPoses3D->points.empty())
        {
            EZLOG(INFO)<< "cloudKeyPoses3D->points.empty() "<<std::endl;
            return true;
        }
...
//        当前帧位姿
    ...
    void addGPSFactor() {
        ...
        if (syncGPS(gpsQueue, thisGPS, timeLaserInfoCur, 1.0 / Config::gpsFrequence)) {
            // GPS too noisy, skip
            ...
            if (abs(noise_x) > Config::gpsCovThreshold || abs(noise_y) > Config::gpsCovThreshold)
                return;
...
//            float gps_z = thisGPS.pose.pose.position.z;
            ...
            curGPSPoint.y = gps_y;
            curGPSPoint.z = gps_z;
            if (pointDistance(curGPSPoint, lastGPSPoint) < Config::gpsDistance)
                return;
            else
...
//--------------        ISAM是否清空
        ...
        //------------------ 历史点云 需要吗？cloudKeyPoses3D comment by sy
...
//        keyframeTimes.push_back(timeLaserInfoStamp.toSec());
    ...
    void publishOdometry() {
        ...
        tf::StampedTransform trans_odom_to_lidar = tf::StampedTransform(
                t_odom_to_lidar, timeLaserInfoStamp, Config::odometryFrame, "lidar_link");
        br.sendTransform(trans_odom_to_lidar);

        if (Config::useGPS) {
            if (gpsTransfromInit && Config::updateOrigin) {
            ...
            fix_msgs.header.frame_id = Config::odometryFrame;
    ...
    void publishFrames() {
        ...
        if (pubCloudRaw.getNumSubscribers() != 0) {
            pcl::PointCloud<PointType>::Ptr cloudOut(
                    new pcl::PointCloud<PointType>());
            pcl::fromROSMsg(cloudInfo.cloud_deskewed, *cloudOut);
            publishCloud(pubCloudRaw, cloudOut, timeLaserInfoStamp, Config::lidarFrame);
        }
            ...
            if (lastSLAMInfoPubSize != cloudKeyPoses6D->size()) {
                ...
                *cloudOut += *laserCloudSurfLastDS;
                slamInfo.key_frame_cloud = publishCloud(ros::Publisher(), cloudOut,
                                                        timeLaserInfoStamp, Config::lidarFrame);
                ...
                *localMapOut += *laserCloudSurfFromMapDS;
                slamInfo.key_frame_map = publishCloud(
                        ros::Publisher(), localMapOut, timeLaserInfoStamp, Config::odometryFrame);
