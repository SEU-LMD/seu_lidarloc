    ...
    IMUPreintegration() {
        ...
        correctionNoise2 = gtsam::noiseModel::Diagonal::Sigmas(
                (gtsam::Vector(6) << 1, 1, 1, 1, 1, 1)
                        .finished());  // rad,rad,rad,m, m, m
        noiseModelBetweenBias = (gtsam::Vector(6) << SensorConfig::imuAccBiasN, SensorConfig::imuAccBiasN,SensorConfig::imuAccBiasN,
...
//    }
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg) {
        ...
        /// 0. initialize system  first scan
        if (systemInitialized == false) {
            // 重置ISAM2优化器
            resetOptimization();

            // pop old IMU message  从imu优化队列中删除当前帧激光里程计时刻之前的imu数据,delta_t=0
            ...
            return;
        }//end if (systemInitialized == false)
        ...
        // 2. after optiization, re-propagate imu odometry preintegration   mark!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    */
    void imuHandler(const gps_imu::ivsensorgpsConstPtr &gnssImu_raw) {
        ...
        EZLOG(INFO)<<"doneFirstOpt = "<<doneFirstOpt<<std::endl;
        double imuTime = ROS_TIME(&thisImu);
