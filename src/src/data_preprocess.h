...
//                    cloudInfo.pointRange[count] = rangeMat(i, j);
    ...
    void DoWork() {
        ...
        while (1) {
           ...
           // if(!isEmpty){
            {
                static long long int lidar_frames = 0;
                static long long int GNSS_frames = 0;
...
//                EZLOG(INFO)<<"DoWork"<<endl;
                ...
                double drqueue_min_ros_time = drqueue_copy.front().timestamp;
                double drqueue_max_ros_time = drqueue_copy.back().timestamp;
...
//                        if(GNSSQueue.front().timestamp - cur_scan->timestamp > 0.5f ){
...
//                gnss_mutex.unlock();//TODO 1111----------
                ...
                EZLOG(INFO) << "GNSSQueue.size() = " << GNSSQueue.size() << std::endl;

                for(int i = 0;i<GNSSQueue.size();++i){
                    if(abs(GNSSQueue[i].timestamp - cur_scan->timestamp) < MappingConfig::gnss_align_threshold){
                ...
                while(GNSSQueue.front().timestamp - cur_scan->timestamp < MappingConfig::gnss_pop_threshold ){
                    GNSSQueue.pop_front();
                }
                gnss_mutex.unlock();
                EZLOG(INFO)<<"find gnss pose num = "<<GNSS_frames<<" / "<<lidar_frames<<" = "<<(float)GNSS_frames/(float)lidar_frames;
