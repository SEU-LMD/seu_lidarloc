...
static double PI2 = 3.14159265358979323846;           /* PI     */
#define PI_OVER_2 (PI2 / 2.0e0)             /* PI over 2 */
#define MAX_LAT1 ((PI2 * 89.99) / 180.0)     /* 89.99 degrees in radians */
#define MAX_DELTA_LONG ((PI2 * 90) / 180.0) /* 90 degrees in radians */
...
{ /* BEGIN Convert_Geodetic_To_Transverse_Mercator */
    ...
    if ((Latitude < -MAX_LAT1) || (Latitude > MAX_LAT1)) { /* Latitude out of range */
        Error_Code |= TRANMERC_LAT_ERROR;
    }
    if (Longitude > PI2) Longitude -= (2 * PI2);
    if ((Longitude < (TranMerc_Origin_Long - MAX_DELTA_LONG)) ||
        (Longitude > (TranMerc_Origin_Long + MAX_DELTA_LONG))) {
        if (Longitude < 0) temp_Long = Longitude + 2 * PI2;
        else
            temp_Long = Longitude;
        if (TranMerc_Origin_Long < 0) temp_Origin = TranMerc_Origin_Long + 2 * PI2;
        else
...
{ /* BEGIN Set_Tranverse_Mercator_Parameters */
    ...
    if ((Central_Meridian < -PI2) || (Central_Meridian > (2 * PI2))) { /* origin longitude out of range */
    ...
    if (!Error_Code) { /* no errors */
        ...
        if (Central_Meridian > PI2) Central_Meridian -= (2 * PI2);
        ...
        Convert_Geodetic_To_Transverse_Mercator(MAX_LAT1, MAX_DELTA_LONG + Central_Meridian, &TranMerc_Delta_Easting,
...
long Convert_Transverse_Mercator_To_Geodetic(double Easting, double Northing, double *Latitude,
    ...
    if (!Error_Code) {
        ...
        *Latitude = ftphi - pow(de, 2) * t10 + pow(de, 4) * t11 - pow(de, 6) * t12 + pow(de, 8) * t13;
        ...
        if ((fabs)(*Latitude) > (90.0 * PI2 / 180.0)) Error_Code |= TRANMERC_NORTHING_ERROR;

        if ((*Longitude) > (PI2)) {
            *Longitude -= (2 * PI2);
            if ((fabs)(*Longitude) > PI2) Error_Code |= TRANMERC_EASTING_ERROR;
        } else if ((*Longitude) < (-PI2)) {
            *Longitude += (2 * PI2);
            if ((fabs)(*Longitude) > PI2) Error_Code |= TRANMERC_EASTING_ERROR;
        }

        if (fabs(dlam) > (9.0 * PI2 / 180) * cos(*Latitude)) { /* Distortion will result if Longitude is more than 9
