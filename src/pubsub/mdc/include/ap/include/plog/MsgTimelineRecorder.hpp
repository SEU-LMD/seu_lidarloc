...
public:
     ...
     * @return if success. return PlogReturnValue::OK
     */
    PlogReturnValue RecordUserGuid(const rbs::plog::UserRawDataType&);
     ...
     * @return if success. return PlogReturnValue::OK
     */
    PlogReturnValue RecordTimeLine(StageID atStage, uint64_t timeStamp, UserRawDataType userDefinedData);
     ...
     */
    PlogReturnValue RecordTimeLine(StageID atStage, uint64_t timeStamp);
     ...
     * @return
     */
    static inline uint64_t ConvertSysTime2NsecU64(const struct timespec& sysTime)
    {
    ...
    MsgTimelineRecorder(MsgTimelineRecorder&&) noexcept;
    MsgTimelineRecorder& operator=(MsgTimelineRecorder&&) noexcept;

    ~MsgTimelineRecorder();

    static PlogRecorderHandle ConvertCppRecorderToCHandle(std::unique_ptr<MsgTimelineRecorder>);

    static std::unique_ptr<MsgTimelineRecorder> ConvertFromCHandle(PlogRecorderHandle);
...
private:
    ...
    friend PlogReturnValue ProfileLogWriter::WriteMsgTimeLine(std::unique_ptr<MsgTimelineRecorder>);
    friend PlogReturnValue ProfileLogWriter::WriteMsgTimeLine(MsgTimelineRecorder);
