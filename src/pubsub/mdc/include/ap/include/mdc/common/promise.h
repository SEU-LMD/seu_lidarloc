 ...
 * Create: 2019-07-30
 */
#ifndef MDC_COMMON_PROMISE_H
#define MDC_COMMON_PROMISE_H
...
public:
     ...
     *
     */
    Promise() : mutex_(), lock_(), stdPromise_(), state_(std::make_shared<internal::State>())
    {}
    ...
    Promise(Promise&& other) noexcept
        : mutex_(), lock_(other.mutex_), stdPromise_(std::move(other.stdPromise_)), state_(std::move(other.state_))
    {
        lock_.unlock();
    }
    ...
    void SetError(E&& error)
    {
        ...
        Result<T, E> r = Result<T, E>::FromError(std::move(error));
        state_->LockCallback();
        stdPromise_.set_value(r);
        state_->FireCallBack();
    ...
    void SetError(E const& error)
    {
        ...
        Result<T, E> r =  Result<T, E>::FromError(error);
        state_->LockCallback();
        stdPromise_.set_value(r);
        state_->FireCallBack();
        state_->UnlockCallback();
...
public:
    /**
     * @brief Construct a new Promise object [SWS_CORE_06341]
     *
     */
    Promise() : mutex_(), lock_(), stdPromiseSpec_(), state_(std::make_shared<internal::State>())
    {}

    /**
     * @brief Copy constructor shall be disabled [SWS_CORE_06350]
     *
     */
    Promise(Promise const&) = delete;

    /**
     * @brief Destructor for Promise objects [SWS_CORE_06349].
     *
     */
    ~Promise() = default;
